 ...
 *
 * Compile: gcc -Wall -o command_executor command_executor.c
...
#define MAX_INPUT   1024 //Max input
#define MAX_ARGS    64 //Max no. of command arguments
...
int parse_command(char *input, char *args[]) {
    ...
    while (token != NULL && argc < MAX_ARGS - 1) {//no more words and max argument reached
        args[argc++] = token;//store words inside array
        token = strtok(NULL, " \t\n");//next word
    }
    args[argc] = NULL;   // execvp requires a NULL-terminated array (so it knows weather argument ends)
    return argc;//return no of arguments
}

int main(void) {
    char input[MAX_INPUT];//store user command
    char *args[MAX_ARGS];//store each word separately
    ...
    while (1) {
        printf("\nmyshell> ");
        fflush(stdout); //force prompt to appear immediately instead of waiting

        // Step 1: Accept a command as input
        if (fgets(input, MAX_INPUT, stdin) == NULL) {//read whole line 
        ...
        int argc = parse_command(input, args);//slpit command
        if (argc == 0) {
            continue;
        }
